        ...
        <v-row dense>
            ...
            <v-col md="9">
                <v-row>
                    ...
                    <v-card nuxt :to="`/products/${p.id}`" color="surface" clas="el ma-2 mb-5 mr-5">
                        ...
                        <v-chip x-small label outlined clas="mr-1" v-for="(t, i) in p.tags" :key="`prod${p.id}-${i}`">{{ t }}</v-chip>
                    </v-card>
                    </v-col>
                </template>
                </v-row>
    ...
    filteredProducts(){
        ...
        return this.products.filter((p) =>{
           ...
           const n = p.name.toLowerCase();
            ...
            return (
                n.includes(s) || price.includes(s) || sprice.includes(s) || r.includes(s)
            );
        })
        }
    }
