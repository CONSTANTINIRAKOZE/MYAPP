<template>
    <v-carousel height="100vh" hide-delimiter-background>
        ...
        <v-carousel-item v-for="(p,i) in sale_items" :key="`sale_items-${i}`">
            ...
            <v-container class="fill-height">
                <v-row dense align="center">
                    <v-col md="7">
                        ...
                        <p class="text-md-h5 text-subtitle-1 primary--text mt-5"></p>
                        <p class="text-md-body-2 mb-7">{{ p.description }}</p>
                        <v-btn nuxt :to="`/products/${p.id}`" depressed color="primary" class="text-capitalize" min-height="40">Check It Out</v-btn>
                        </div>
                    </v-col>
                </v-row>
            </v-container></v-img>
